package dev.abhinav.run.presentation.active_run.service
...
class ActiveRunService : Service() {
    ...
    private val baseNotification by lazy {
        NotificationCompat.Builder(applicationContext, CHANNEL_ID)
            .setSmallIcon(dev.abhinav.core.presentation.designsystem.R.drawable.logo)
    ...
    private fun createNotificationChannel() {
        if(Build.VERSION.SDK_INT >= 26) {
            val channel = NotificationChannel(CHANNEL_ID, getString(R.string.active_run), NotificationManager.IMPORTANCE_DEFAULT)
